...
void myremdir(char *dirname)
{
    ...
    while ((path_dir = strtok_r(rest, "/", &rest)))
    {
        ...
        for (int i = 0; i <= DIRENTRYCOUNT; i++)
        {
            if (strcmp(path_dir, navigating_directory.dir.entrylist[i].name) == 0)
                break;
    ...
    { //printf("I am inside");
        if (strcmp(dirname + pos_last_slash, navigating_directory.dir.entrylist[i].name) == 0)
        {
            ...
            for (int j = 0; j < BLOCKSIZE; j++)
                virtualDisk[navigating_directory.dir.entrylist[i].firstblock].data[j] = '\0';

            for (int j = i; j < navigating_directory.dir.nextEntry; j++)
                navigating_directory.dir.entrylist[j] = navigating_directory.dir.entrylist[j + 1];
...
void myremove(char *pathfile)
{
    ...
    for (int i = 0; i <= DIRENTRYCOUNT; i++)
    {
        if (strcmp(pathfile + pos_last_slash, navigating_dir.dir.entrylist[i].name) == 0)
        {
            ...
            for (int j = 0; j < BLOCKSIZE; j++)
                virtualDisk[navigating_dir.dir.entrylist[i].firstblock].data[j] = '\0';
            ...
            for (int j = i; j < navigating_dir.dir.nextEntry; j++)
                navigating_dir.dir.entrylist[j] = navigating_dir.dir.entrylist[j + 1];
...
char **mylistdir(char *path)
{
    ...
    for (int i = 0; i < 10; i++)
        ptr_ptr_file_list[i] = malloc(256 * sizeof(char));
